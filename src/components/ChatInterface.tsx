...
export function ChatInterface() {
  ...
  const getLastUsedModel = (): AIModel => {
    ...
    if (savedModel) {
      try {
        ...
        logger.warn('Failed to parse saved model, using default');
      }
    }
  ...
  useEffect(() => {
    const initializeConversation = () => {
      ...
      logger.info('Initializing conversation system:', { currentId, conversationCount: conversations.length });
      ...
      if (currentId && conversations.find(c => c.id === currentId)) {
        ...
        logger.info('Loaded current conversation:', currentId, 'with', conversationMessages.length, 'messages');
        ...
        setMessages(conversationMessages);
        logger.info('Loaded most recent conversation:', mostRecent.id, 'with', conversationMessages.length, 'messages');
      } else {
        // No conversations exist - start fresh (don't create one yet)
        setCurrentConversationId(null);
        setMessages([]);
        logger.info('No conversations found - starting fresh');
      }
    };
  ...
  useEffect(() => {
    const handleConversationSwitched = (event: CustomEvent) => {
      ...
      logger.debug('Conversation switched event:', conversationId);
      ...
      if (currentConversationId && messages.length > 0) {
        ...
        if (currentConversation) {
          ...
          logger.debug('Saved current conversation before switching');
        }
      }
      ...
      logger.info('Switched to conversation:', conversationId, 'with', newMessages.length, 'messages');
    };

    const handleConversationDeleted = (event: CustomEvent) => {
      const { conversationId, data } = event.detail;
      logger.debug('Conversation deleted event:', conversationId, data);
    ...
    if (currentConversationId && messages.length > 0) {
      ...
      if (conversation) {
        ...
        const timeoutId = setTimeout(() => {
          ...
          logger.debug('Auto-saved conversation:', currentConversationId, 'with', messages.length, 'messages');
  ...
  useEffect(() => {
    ...
    logger.info(`Changed model to ${defaultModelForProvider.name} from provider ${selectedProvider}`);
  }, [selectedProvider]);
  ...
  const convertMessagesToApiFormat = (messages: Message[]) => {
    ...
    logger.debug('Converting messages to API format:', {
  ...
  const handleSendMessageInternal = async (content: string, attachments?: FileAttachment[], isRegeneration: boolean = false, initialMessages?: Message[], previousRegenerationCount?: number) => {
    ...
    if (isNew || currentConversationId !== conversation.id) {
      ...
      if (isNew) {
        setMessages([]);
        logger.info('Created new conversation:', conversation.id, conversation.title);
      } else {
        const existingMessages = getConversationMessages(conversation.id);
        setMessages(existingMessages);
        logger.debug('Switched to existing conversation:', conversation.id);
      }
    }
  ...
  const handleModelChange = (model: AIModel) => {
    logger.info(`Changing model to: ${model.name} from provider ${model.provider}`);
  ...
  const handleProviderChange = (provider: string) => {
    logger.info(`Changing provider to: ${provider}`);
  ...
  useEffect(() => {
    ...
    if (configuredProviders.length > 0) {
      logger.info('Configured AI providers:', configuredProviders);
    } else {
      logger.info('No API keys configured. Using simulated responses.');
    }
  }, []);
          ...
          messages.map((message, index) => (
            <MessageItem 
              ...
              onBranchCreated={(conversationId) => {
                ...
                logger.debug('Branch created:', conversationId);
              }}
            />
          ))
        )}
