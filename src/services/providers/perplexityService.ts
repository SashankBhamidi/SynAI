...
export class PerplexityService extends BaseProviderService {
  ...
  async sendRequest(options: AiRequestOptions): Promise<string> {
    ...
    try {
      ...
      logger.debug('Sending to Perplexity with messages:', JSON.stringify(requestMessages, null, 2));
      ...
      logger.debug('Perplexity raw response:', responseText.substring(0, 200));
      ...
      try {
        ...
        logger.error('Failed to parse Perplexity response as JSON:', error);
      ...
      logger.debug('Perplexity response content:', structuredResponse);
      ...
      if (!this.apiKey && error instanceof TypeError && error.message.includes('Failed to fetch')) {
        logger.info('Network error with no API key - using simulated response');
        return this.generateSimulatedResponse('Synthesis AI', model, options.messages);
      }
  ...
  private parsePerplexityResponse(content: string): string {
    logger.debug('Original Perplexity response:', content);
    ...
    logger.debug('Final processed content:', finalContent);
    return finalContent;
  }
  ...
  private extractSources(content: string): { processedContent: string; sourceUrls: Record<number, string> } {
    ...
    if (sourcesMatch) {
      logger.debug('Found sources section:', sourcesMatch[1]);
      ...
      logger.debug('Parsed sources:', sources);
      ...
      logger.debug('Source URLs mapping:', sourceUrls);
    }
    ...
    if (citationMatches && Object.keys(sourceUrls).length === 0) {
      logger.debug('Found citations without sources section, creating placeholder links');
  ...
  private convertMessagesToPerplexityFormat(messages: ApiMessage[]) {
    logger.debug('Converting messages to Perplexity format:', JSON.stringify(messages, null, 2));
    ...
    logger.debug('Converted to Perplexity format:', JSON.stringify(convertedMessages, null, 2));
  ...
  async validateKey(_apiKey: string): Promise<boolean> {
    try {
      ...
      logger.error('Failed to validate Perplexity API key:', error);
      return true;
    }
  }
}
