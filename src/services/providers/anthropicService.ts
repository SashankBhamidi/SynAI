...
export class AnthropicService extends BaseProviderService {
  ...
  async sendRequest(options: AiRequestOptions): Promise<string> {
    ...
    try {
      ...
      if (!this.apiKey || !this.client || options.simulateResponse) {
        logger.info('No Anthropic API key configured, using simulated response');
  ...
  private convertMessagesToAnthropicFormat(messages: ApiMessage[]) {
    logger.debug('Converting messages to Anthropic format:', JSON.stringify(messages, null, 2));
      ...
      .map(msg => {
        ...
        if (Array.isArray(msg.content)) {
          ...
          if (anthropicContent.length === 0) {
            logger.warn('Empty multimodal content detected, using fallback');
            return {
          ...
          if (!content.trim()) {
            logger.warn('Empty message content detected, using fallback');
            return {
              role: msg.role === 'assistant' ? 'assistant' as const : 'user' as const,
              content: msg.role === 'assistant' ? 'I understand.' : 'Hello'
            };
    ...
    logger.debug('Converted to Anthropic format:', JSON.stringify(convertedMessages, null, 2));
    return convertedMessages;
  }

  private validateMessageOrder(messages: Array<{role: string; content: string | unknown[]}>) {
