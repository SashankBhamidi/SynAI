...
export class OpenAIService extends BaseProviderService {
  ...
  async sendRequest(options: AiRequestOptions): Promise<string> {
    ...
    try {
      ...
      logger.debug('Sending to OpenAI with messages:', JSON.stringify(openaiMessages, null, 2));
      
      // Validate message format before sending
      openaiMessages.forEach((msg, index) => {
        if (typeof msg.content !== 'string' && !Array.isArray(msg.content)) {
          logger.error(`Message ${index} has invalid content type:`, typeof msg.content, msg.content);
        }
      });
      ...
      logger.debug('OpenAI raw response:', responseText.substring(0, 200));
